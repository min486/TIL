...
```python
dir = Director.objects.get(name='봉준호')

print('id :', dir.id)
print('name :', dir.name)
print('debut :', dir.debut)
print('country :', dir.country)
```
...
for mov in movies.order_by('-opening_date'):
    print(mov.director.name, mov.genre.title, mov.title, mov.opening_date, mov.running_time, mov.screening)
```
...
```python
movies = Movie.objects.all()
mov = movies.order_by('opening_date')[0]
...
```python
movies = Movie.objects.filter(screening = 'True')
...
movie = Movie.objects.filter(director=bong)

for mov in movie.order_by('opening_date'):
    print(mov.director.name, mov.genre.title, mov.title, mov.opening_date, mov.running_time, mov.screening)
```
...
result = movie.order_by('opening_date')[1]

print(result.director.name, result.genre.title, result.title, result.opening_date, result.running_time, result.screening)
```
...
```python
movie = Movie.objects.filter(running_time__gt=119)

for mov in movie.order_by('running_time'):
    print(mov.director.name, mov.genre.title, mov.title, mov.opening_date, mov.running_time, mov.screening)
```
...
```python
movie = Movie.objects.filter(running_time__gte=119)
...
for i in Movie.objects.filter(opening_date__gt=date).order_by('opening_date'):
...
movie = Movie.objects.filter(director=bong, genre=drama)
...
```python
movie = Movie.objects.exclude(director=1)
